...
def translate_essay_with_gemini(essay_title, essay_text):
    ...
    4. 최종 아웃풋은 Markdown 형식으로 제공해주세요. 예를 들어, 제목은 '# 제목' 과 같이 마크다운 제목 레벨 1로 시작해야 합니다. 그리고 본문을 작성할 때 <br />만큼 Markdown 출력에서도 개행을 제대로 해주세요.
    ...
    {{감사의 말입니다. 주석 앞에 옵니다. 원문에 '감사의 말'이라는 소제목이 명시된 경우에만 소제목을 넣어주세요. 그렇지 않으면 생략하고 감사의 말 내용만 번역하면 됩니다. ---등의 가로 구분선 표시도 원문에 있을 때만 쓰세요. optional}}

    {{[^n]: n번째 주석에 관한 설명. markdown-it-footnote 를 이용하기 위해 콜론이 꼭 있어야 함. 출력의 가장 마지막에 위치해야 합니다. 별도 섹션으로 나누지 않습니다. 렌더링 시 자동으로 구분됩니다. 따라서 절대로 --- 같은 가로 구분선 표시를 따로 넣지 마세요. optional}}
    --- 예시 포맷 끝 ---
